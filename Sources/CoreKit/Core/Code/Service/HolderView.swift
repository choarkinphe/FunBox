...
//  Store
//
//  Created by choarkinphe on 2020/6/19.
//  Copyright © 2020 Konnech. All rights reserved.
//

import UIKit

public protocol HolderType {
    var holderType: UIView.Holder? { get }
}

extension HolderType {
...
extension UIView: HolderNamespaceWrappable {
    public struct Holder {
        ...
        public static var faild = Holder(imageName: "holder_image_faild", description: "加载失败")
        public static var load = Holder(imageName: "holder_image_load", description: "加载中")
        public static var empty = Holder(imageName: "holder_image_empty", description: "暂无数据")
        public static var fix = Holder(imageName: "holder_image_fix", description: "修复中")
        public static var notFound = Holder(imageName: "holder_image_notFound", description: "加载异常")
    }
    
    fileprivate struct Key {
        static var identifier: String = "com.CoreKit.holder.identifier"
        static var handle: String = "com.CoreKit.holder.handle"
    }
...
fileprivate class HolderView: UIView {
    ...
    var holder: Holder {
        didSet {
            ...
            if let image = UIImage(named: holder.imageName) {
                ...
                // 项目中未找到，再获取包内数据
                imageView.image = UIImage(named: holder.imageName, in: CoreKit.bundle, compatibleWith: nil)
            }
    ...
    override init(frame: CGRect) {
        ...
        descriptionLabel.font = Theme.Font.default
...
extension Service.Result: HolderType {
    public var holderType: UIView.Holder? {
        
        switch status {
        case .notFound:
            return .notFound
        case .error,.noPermission,.forbidden,.unauthorized:
            return .faild
        default:
            return nil
        }
