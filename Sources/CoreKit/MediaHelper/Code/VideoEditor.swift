...
//            })
    ...
    @objc private func format(sender: UIBarButtonItem) {
        ...
        if slider.sliderDidChanged == false, mode == .edit {
            ...
            //            if asset.duration.seconds <= slider.max_duration,
            //                min(source.pixelWidth, source.pixelHeight) <= 720 {
            //                // 分辨率小于720P的话，直接忽略掉之前的转码过程
            //                DispatchQueue.main.async {
            //                    self.completion?((self.imageView.image,self.source))
            //                }
            //            } else {
            //
            // 新增模式下，直接转码操作
            HZHUD.toast(.loading, message: "转码中，请稍候...")
            ...
            helper?.progress({ (progress) in
                ...
                HZHUD.dismissActivity()
                // 保存到相册
                HZHUD.toast(.loading, message: "正在保存...")
